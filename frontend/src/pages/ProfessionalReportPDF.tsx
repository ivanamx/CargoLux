...
const ReportePDF: React.FC = () => {
  ...
  const loadProjectData = async () => {
    try {
      ...
      // Determinar si es proyecto Panasonic o no
      const isPanasonicProject = projectData?.client?.toLowerCase().includes('panasonic');
      let checkpoints = [];
      
      if (isPanasonicProject) {
        // Cargar datos del reporte (checkpoints de Panasonic)
        console.log('ReportePDF: Proyecto Panasonic - cargando checkpoints');
        ...
        if (reportResponse.ok) {
          checkpoints = await reportResponse.json();
          console.log('ReportePDF: Checkpoints Panasonic recibidos:', checkpoints.length);
        } else {
          console.warn('ReportePDF: No se pudieron cargar checkpoints Panasonic');
        }
      } else {
        // Para proyectos no-Panasonic, usar datos de scanned_codes
        console.log('ReportePDF: Proyecto no-Panasonic - cargando scanned_codes');
        const scannedCodesResponse = await fetch(`/api/scanned-codes/project/${projectId}`, {
          headers: {
            'Authorization': `Bearer ${getToken()}`,
            'Content-Type': 'application/json'
          }
        ...
        if (scannedCodesResponse.ok) {
          const scannedCodes = await scannedCodesResponse.json();
          // Convertir scanned_codes a formato similar a checkpoints
          checkpoints = scannedCodes.map((sc: any) => ({
            scanned_code: sc.code,
            status: sc.status || 'ok',
            timestamp: sc.created_at,
            user_id: sc.user_id,
            user: sc.user
          }));
          console.log('ReportePDF: Scanned codes convertidos a checkpoints:', checkpoints.length);
        } else {
          console.warn('ReportePDF: No se pudieron cargar scanned_codes');
        }
      }
      ...
      // Calcular estadísticas según el tipo de proyecto
      let totalParts, successfulParts, failedParts;
      let uniqueBatteries = [];
      
      if (isPanasonicProject) {
        // Para Panasonic: contar solo baterías únicas (códigos que empiezan con "BAT")
        const batteryCheckpoints = checkpoints.filter((cp: any) => cp.scanned_code.startsWith('BAT'));
        uniqueBatteries = [...new Set(batteryCheckpoints.map((cp: any) => cp.scanned_code))];
        totalParts = uniqueBatteries.length;
        
        successfulParts = [...new Set(
          batteryCheckpoints
        ...
        // Para otros proyectos: usar datos de scanned_codes directamente
        if (checkpoints.length > 0) {
          const uniqueCodes = [...new Set(checkpoints.map((cp: any) => cp.scanned_code))];
          totalParts = uniqueCodes.length;
          ...
          successfulParts = successfulCodes.length;
          ...
          failedParts = failedCodes.length;
        } else {
          // Fallback a datos del proyecto si no hay scanned_codes
          totalParts = projectData?.total_parts || 0;
          successfulParts = projectData?.completed_parts || 0;
          failedParts = 0; // No podemos calcular fallidos sin scanned_codes
        }
      }
      
      // Estadísticas de hoy - según tipo de proyecto
      const todayRelevantCheckpoints = isPanasonicProject 
        ? todayCheckpoints.filter((cp: any) => cp.scanned_code.startsWith('BAT'))
        : todayCheckpoints;

      // Calcular estadísticas por categoría - según tipo de proyecto
      const relevantCheckpoints = isPanasonicProject 
        ? checkpoints.filter((cp: any) => cp.scanned_code.startsWith('BAT'))
        : checkpoints;
        
      const categoryStats = relevantCheckpoints.reduce((acc: any, cp: any) => {
        if (cp.categorie) {
      ...
      // Calcular estadísticas de hoy por categoría - según tipo de proyecto
      const todayCategoryStats = todayRelevantCheckpoints.reduce((acc: any, cp: any) => {
        if (cp.categorie) {
          const categories = cp.categorie.split(',').map((cat: string) => cat.trim());
          const scannedCode = cp.scanned_code;
          categories.forEach((cat: string) => {
      ...
      const technicianStatsFromCheckpoints = relevantCheckpoints.reduce((acc: any, cp: any) => {
      ...
      const todayUniqueCodes = [...new Set(todayRelevantCheckpoints.map((cp: any) => cp.scanned_code))];
      const todayTotalParts = todayUniqueCodes.length;
      
      const todaySuccessfulParts = [...new Set(
        todayRelevantCheckpoints
          .filter((cp: any) => cp.status === 'ok')
          .map((cp: any) => cp.scanned_code)
      )].length;
      ...
      // Tiempo promedio por parte en minutos (dato mock)
      const avgTimePerPart = 45; // Dato mock: 45 minutos por parte
      ...
      console.log('- Códigos únicos encontrados:', isPanasonicProject ? 
        [...new Set(checkpoints.filter((cp: any) => cp.scanned_code.startsWith('BAT')).map((cp: any) => cp.scanned_code))] : 
        [...new Set(checkpoints.map((cp: any) => cp.scanned_code))]);
      console.log('- Total checkpoints relevantes:', relevantCheckpoints.length);
      ...
      console.log('- Tiempo promedio por parte (min):', avgTimePerPart);
      console.log('ReportePDF: Tiempo promedio por parte:');
      console.log('- Tiempo promedio desde quality_questions (seg):', avgBoxTimeFromQuality);
      console.log('- Tiempo promedio final (min):', avgTimePerPart);
      ...
      console.log('- Técnicos finales:', technicianStats);
        ...
        // Cargar issues del proyecto
        console.log('ReportePDF: Cargando issues del proyecto...');
        let issues = [];
        try {
          const issuesResponse = await fetch(`/api/issues/project/${projectId}`, {
            headers: {
              'Authorization': `Bearer ${getToken()}`,
              'Content-Type': 'application/json'
            }
          ...
          if (issuesResponse.ok) {
            ...
            console.log('ReportePDF: Issues del proyecto encontrados:', issues);
          } else {
            console.warn('ReportePDF: No se pudieron cargar los issues del proyecto');
          }
        } catch (error) {
          console.warn('ReportePDF: Error cargando issues del proyecto:', error);
        }
      ...
      const processedData: ReportData = {
        ...
        todayStats: {
          ...
          avgTimePerPart: avgTimePerPart // 45 minutos por parte (dato mock)
        },
                  ...
                  <Text size="sm" c="dimmed">
                    ...
                    <Grid.Col span={6}>
                      <Card withBorder p="md" style={{ backgroundColor: 'white', border: '1px solid #e9ecef' }}>
                        <Group justify="space-between" align="center" mb="xs">
                      <Text size="xl" fw={700} c="orange.6">
                        {reportData.todayStats.avgTimePerPart}
                      </Text>
                      <Group gap="xs" align="center">
                        <IconTrendingUp size={14} color="var(--mantine-color-orange-6)" />
                        <Text size="xs" c="orange" fw={500}>
                              →{reportData.todayStats.projectedPartsUntil18} partes a las 18:00
                            </Text>
                          </Group>
                        </Group>
                        <Text size="sm" c="dimmed">
                          Tiempo por parte (min)
                        </Text>
                </Card>
              </Grid.Col>
            </Grid>
                  ...
                  <Stack gap="md">
                    <Card withBorder p="md" style={{ backgroundColor: 'white', border: '1px solid #e9ecef' }}>
                      <Text size="sm" fw={600} c="dark" mb="sm">
                        Lista de Problemas - Proyecto {project.name}
                      </Text>
                        ...
                        <div style={{ 
                          ...
                          {reportData.issues.map((issue, index) => (
                            <React.Fragment key={index}>
                              <div style={{ 
                                ...
                                backgroundColor: issue.status === 'pendiente' ? '#fef2f2' : 
                                               issue.status === 'en-revision' ? '#fffbeb' : '#f0fdf4'
                              }}>
                                      <Badge 
                                  size="xs" 
                                        color={issue.status === 'pendiente' ? 'red' : 
                                               issue.status === 'en-revision' ? 'yellow' : 'green'}
                                        variant="light"
                                      >
                                  {issue.type === 'sin_bateria' ? 'Sin Batería' :
                                   issue.type === 'sin_llaves' ? 'Sin Llaves' :
                                   issue.type === 'software' ? 'Software' :
                                   issue.type === 'hardware' ? 'Hardware' :
                                   issue.type === 'conectividad' ? 'Conectividad' :
                                   issue.type === 'calidad' ? 'Calidad' :
                                   issue.type === 'seguridad' ? 'Seguridad' :
                                   issue.type === 'mantenimiento' ? 'Mantenimiento' :
                                   issue.type === 'otro' ? 'Otro' :
                                   issue.type}
                                      </Badge>
                                  </div>
                              <div style={{ 
                                ...
                                {issue.part_number_vin ? issue.part_number_vin.slice(-6) : 'N/A'}
                              </div>
                              <div style={{ 
                                padding: '8px', 
                                borderBottom: '1px solid #e9ecef',
                          ...
                          <Text size="sm" c="dimmed" ta="center" py="xl">
                            No se encontraron problemas reportados para este proyecto
                          </Text>
                        )}
                    </Card>
                        ...
                        <Grid.Col span={4}>
                          <div style={{ textAlign: 'center' }}>
                            <Text size="xl" fw={700} c="red">
                              {reportData.issues ? reportData.issues.filter(i => i.status === 'pendiente').length : 0}
                            </Text>
                        ...
                        <Grid.Col span={4}>
                          <div style={{ textAlign: 'center' }}>
                            <Text size="xl" fw={700} c="yellow">
                              {reportData.issues ? reportData.issues.filter(i => i.status === 'en-revision').length : 0}
                            </Text>
                            <Text size="xs" c="dimmed">En Revisión</Text>
                          </div>
                        </Grid.Col>
                        <Grid.Col span={4}>
                          <div style={{ textAlign: 'center' }}>
                            <Text size="xl" fw={700} c="green">
                              {reportData.issues ? reportData.issues.filter(i => i.status === 'resuelto').length : 0}
                            </Text>
